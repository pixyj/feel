...
def create_payload(commit, output_zip_dir):
    ...
    exec_command(command)
    command = "cp {} {}".format('dist/favicon.png', parent_dir)
    exec_command(command)
    dir_name = 'dist-{}'.format(commit)
    command = 'zip -r {}/dist-{}.zip {}'.format(output_zip_dir,
                                                commit, dir_name)
    exec_command(command)
    cleanup_min_files()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-z', '--output_zip_dir',
                        help='Where do you wanna store the zip?',
                        required=True)
